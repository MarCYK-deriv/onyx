    ...
    def process(
        ...
        if MULTI_TENANT:
            ...
            if tenant_id != POSTGRES_DEFAULT_SCHEMA and tenant_id is not None:
                ...
                short_tenant = (
                    tenant_display[:8] if len(
                        tenant_display) > 8 else tenant_display
                )
...
def setup_logger(
    ...
    logger.notice = lambda msg, *args, **kwargs: logger.log(
        logging.getLevelName("NOTICE"), msg, *args, **kwargs)  # type: ignore
...
def print_loggers() -> None:
    ...
    for logger in loggers:
        ...
        print(
            f"Logger: '{logger.name}' (Level: {logging.getLevelName(logger.level)})")
        if logger.handlers:
